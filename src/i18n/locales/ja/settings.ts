...
export default {
  ...
  "settings.clickThrough.hint":
    "オンにしても、後ろのアプリにクリックを渡すのは左タブバーの透明部分（余白、タブ同士の隙間、未選択タブの半透明部分）だけです。アイコンにはヒット領域があり、選択中のタブとコンテンツ領域は常にクリックを受け取り、ウィンドウの四辺はリサイズ用のつかみ領域として残るのでサイズ変更もできます。現在の状態は下のライブ表示に出ます。",
