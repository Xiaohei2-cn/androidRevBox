...
export default {
  ...
  "settings.clickThrough.hint":
    "Когда включено, клики по по-настоящему прозрачным местам (прозрачная область и сами «зубцы» панели вкладок, промежутки между карточками, поля, вне скруглённых углов) уходят в приложение позади окна. Карточки, кнопки, поля и текст по-прежнему достаются этому инструменту. У каждой вкладки остаётся область попадания по иконке, а строка заголовка всегда плотная — можно вернуться сюда и выключить.",
